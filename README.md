...
- cron 엔드포인트는 실패 시 `500`과 오류 메시지를 반환합니다.

## Manual Endpoint Reproduction
로컬에서 Vercel 엔드포인트 계약을 흉내 내려면 WSGI app을 직접 호출하거나, 테스트 코드처럼 synthetic environ을 사용할 수 있습니다. 운영에서는 Vercel이 `/api/cron`에 `Authorization: Bearer <CRON_SECRET>` 헤더를 붙여 호출합니다.
